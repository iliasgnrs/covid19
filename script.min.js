...
return vb};parseViewBoxString=function(string){var vb;vb=string.replace("\s+"," ").split(" ");return vb={x:parseFloat(vb[0]),y:parseFloat(vb[1]),width:parseFloat(vb[2]),height:parseFloat(vb[3])}};getViewBoxCoordinatesFromEvent=function(svgRoot,event){var ctm,foo,pos;foo={x:null,y:null};if(event.type==="touchstart"||event.type==="touchmove"){if((event.originalEvent!=null)&&(event.touches==null)){foo.x=event.originalEvent.touches[0].clientX;foo.y=event.originalEvent.touches[0].clientY}else{foo.x=event.touches[0].clientX;foo.y=event.touches[0].clientY}}else{if(event.clientX!=null){foo.x=event.clientX;foo.y=event.clientY}else{foo.x=event.originalEvent.clientX;foo.y=event.originalEvent.clientY}}
pos=svgRoot.createSVGPoint();pos.x=parseInt(foo.x,10);pos.y=parseInt(foo.y,10);ctm=svgRoot.getScreenCTM();ctm=ctm.inverse();pos=pos.matrixTransform(ctm);return pos};return $.fn.svgPanZoom=function(options){var ret;ret=[];this.each(function(){var $animationDiv,dragStarted,horizontalSizeIncrement,key,opts,preventClick,value,vb,verticalSizeIncrement,viewBox;opts=$.extend(!0,{},defaultOptions,options);opts.$svg=$(this);if(opts.animationTime==null){opts.animationTime=0}
opts.$svg[0].setAttribute("preserveAspectRatio","xMidYMid meet");vb=$.extend({},this.viewBox.baseVal);if(vb.x==null){vb.x=0}
if(vb.y==null){vb.y=0}
if(vb.width==null){vb.width=0}
if(vb.height==null){vb.height=0}
if(opts.initialViewBox!=null){if(typeof opts.initialViewBox==="string"){vb=parseViewBoxString(opts.initialViewBox)}else if(typeof opts.initialViewBox==="object"){vb=$.extend({},defaultViewBox,opts.initialViewBox)}else{throw "initialViewBox is of invalid type"}}else if(vb.x===0&&vb.y===0&&vb.width===0&&vb.height===0){vb=defaultViewBox}
viewBox=vb;opts.initialViewBox=$.extend({},viewBox);if(opts.limits==null){horizontalSizeIncrement=viewBox.width*0.15;verticalSizeIncrement=viewBox.height*0.15;opts.limits={x:viewBox.x-horizontalSizeIncrement,y:viewBox.y-verticalSizeIncrement,x2:viewBox.x+viewBox.width+horizontalSizeIncrement,y2:viewBox.y+viewBox.height+verticalSizeIncrement}}
...
if(animationTime>0){$animationDiv.css({left:viewBox.x+"px",top:viewBox.y+"px",width:viewBox.width+"px",height:viewBox.height+"px"})}
viewBox={x:x!=null?x:viewBox.x,y:y!=null?y:viewBox.y,width:width?width:viewBox.width,height:height?height:viewBox.height};viewBox=checkLimits(viewBox,this.limits);if(animationTime>0){$animationDiv.stop().animate({left:viewBox.x,top:viewBox.y,width:viewBox.width,height:viewBox.height},{duration:animationTime,easing:"linear",step:(function(value,properties){var $div;$div=$animationDiv;this.$svg[0].setAttribute("viewBox",($div.css("left").slice(0,-2))+" "+($div.css("top").slice(0,-2))+" "+($div.css("width").slice(0,-2))+" "+($div.css("height").slice(0,-2)))}).bind(this)})}else{this.$svg[0].setAttribute("viewBox",viewBox.x+" "+viewBox.y+" "+viewBox.width+" "+viewBox.height)}};opts.panLeft=function(amount,animationTime){if(amount==null){amount=this.panFactor}
if(animationTime==null){animationTime=this.animationTime}
this.panRight(-amount,animationTime)};opts.panRight=function(amount,animationTime){if(amount==null){amount=this.panFactor}
if(animationTime==null){animationTime=this.animationTime}
this.setViewBox(viewBox.x+amount,null,null,null,animationTime)};opts.panUp=function(amount,animationTime){if(amount==null){amount=this.panFactor}
if(animationTime==null){animationTime=this.animationTime}
this.panDown(-amount,animationTime)};opts.panDown=function(amount,animationTime){if(amount==null){amount=this.panFactor}
if(animationTime==null){animationTime=this.animationTime}
this.setViewBox(null,viewBox.y+amount,null,null,animationTime)};opts.zoomIn=function(amount,animationTime){if(amount==null){amount=this.zoomFactor}
if(animationTime==null){animationTime=this.animationTime}
this.zoomOut(-amount,animationTime);$('.map-overlay').addClass('active')};opts.zoomOut=function(amount,animationTime){var center,newHeight,newWidth;if(amount==null){amount=this.zoomFactor}
if(animationTime==null){animationTime=this.animationTime}
if(amount===0){return}else if(amount<0){amount=Math.abs(amount);newWidth=viewBox.width/(1+amount);newHeight=viewBox.height/(1+amount)}else{newWidth=viewBox.width*(1+amount);newHeight=viewBox.height*(1+amount)}
center={x:viewBox.x+viewBox.width/2,y:viewBox.y+viewBox.height/2};this.setViewBox(center.x-newWidth/2,center.y-newWidth/2,newWidth,newHeight,animationTime);$('.map-overlay').removeClass('active')};opts.setCenter=function(x,y,animationTime){if(animationTime==null){animationTime=this.animationTime}
this.setViewBox(x-viewBox.width/2,y-viewBox.height/2,viewBox.width,viewBox.height,animationTime)};for(key in opts){if(!hasProp.call(opts,key))continue;value=opts[key];if(typeof value==="function"){opts.key=value.bind(opts)}}
opts.$svg.on("mousewheel DOMMouseScroll MozMousePixelScroll",(function(ev){var delta,minHeight,minWidth,newMousePosition,newViewBox,newcenter,oldDistanceFromCenter,oldMousePosition,oldViewBox,oldcenter,reductionFactor;delta=parseInt(ev.originalEvent.wheelDelta||-ev.originalEvent.detail);if(delta===0||opts.events.mouseWheel!==!0){return}
oldViewBox=this.getViewBox();ev.preventDefault();ev.stopPropagation();oldMousePosition=getViewBoxCoordinatesFromEvent(this.$svg[0],ev);oldcenter={x:viewBox.x+viewBox.width/2,y:viewBox.y+viewBox.height/2};oldDistanceFromCenter={x:oldcenter.x-oldMousePosition.x,y:oldcenter.y-oldMousePosition.y};if(delta>0){this.zoomIn(void 0,0);minWidth=this.initialViewBox.width/this.maxZoom;minHeight=this.initialViewBox.height/this.maxZoom;if(viewBox.width<minWidth){reductionFactor=minWidth/viewBox.width;viewBox.width=minWidth;viewBox.height=viewBox.height*reductionFactor}
if(viewBox.height<minHeight){reductionFactor=minHeight/viewBox.height;viewBox.height=minHeight;viewBox.width=viewBox.width*reductionFactor}}else{this.zoomOut(void 0,0)}
newMousePosition=getViewBoxCoordinatesFromEvent(this.$svg[0],ev);newcenter={x:oldcenter.x+(oldMousePosition.x-newMousePosition.x),y:oldcenter.y+(oldMousePosition.y-newMousePosition.y)};this.setCenter(newcenter.x,newcenter.y,0);newViewBox=this.getViewBox();this.setViewBox(oldViewBox.x,oldViewBox.y,oldViewBox.width,oldViewBox.height,0);this.setViewBox(newViewBox.x,newViewBox.y,newViewBox.width,newViewBox.height)}).bind(opts));opts.$svg.dblclick((function(ev){if(opts.events.doubleClick!==!0){return}
ev.preventDefault();ev.stopPropagation();return this.zoomIn()}).bind(opts));opts.$svg[0].addEventListener("click",function(ev){var preventClick;if(preventClick){preventClick=!1;ev.stopPropagation();return ev.preventDefault()}},!0);dragStarted=!1;preventClick=!1;opts.$svg.on("mousedown touchstart",(function(ev){var $body,domBody,initialViewBox,mouseMoveCallback,mouseUpCallback,oldCursor;if(dragStarted){return}
if(opts.events.drag!==!0||(ev.type==="mousedown"&&ev.which!==1)){return}
dragStarted=!0;preventClick=!1;initialViewBox=$.extend({},viewBox);$body=$(window.document.body);domBody=$body[0];oldCursor=$body.css("cursor");if(this.events.dragCursor!=null){$body.css("cursor",this.events.dragCursor)}
mouseMoveCallback=(function(ev2){var currentMousePosition,initialMousePosition;ev2.preventDefault();ev2.stopPropagation();initialMousePosition=getViewBoxCoordinatesFromEvent(this.$svg[0],ev);currentMousePosition=getViewBoxCoordinatesFromEvent(this.$svg[0],ev2);if(Math.sqrt(Math.pow(ev.pageX+ev2.pageX,2)+Math.pow(ev.pageY+ev2.pageY,2))>3){preventClick=!0}
this.setViewBox(initialViewBox.x+initialMousePosition.x-currentMousePosition.x,initialViewBox.y+initialMousePosition.y-currentMousePosition.y,null,null,0)}).bind(opts);mouseUpCallback=(function(ev2){if(ev2.type==="mouseout"&&ev2.target!==ev2.currentTarget){return}
ev2.preventDefault();ev2.stopPropagation();domBody.removeEventListener("mousemove",mouseMoveCallback,!0);domBody.removeEventListener("touchmove",mouseMoveCallback,!0);domBody.removeEventListener("mouseup",mouseUpCallback,!0);domBody.removeEventListener("touchend",mouseUpCallback,!0);domBody.removeEventListener("touchcancel",mouseUpCallback,!0);domBody.removeEventListener("mouseout",mouseUpCallback,!0);if(this.events.dragCursor!=null){$body.css("cursor",oldCursor)}
dragStarted=!1}).bind(opts);domBody.addEventListener("mousemove",mouseMoveCallback,!0);domBody.addEventListener("touchmove",mouseMoveCallback,!0);domBody.addEventListener("mouseup",mouseUpCallback,!0);domBody.addEventListener("touchcancel",mouseUpCallback,!0);domBody.addEventListener("mouseout",mouseUpCallback,!0)}).bind(opts));opts.setViewBox(vb.x,vb.y,vb.width,vb.height,0);ret.push(opts)});if(ret.length===0){return null}
if(ret.length===1){return ret[0]}else{return ret}}})(jQuery)}).call(this);(function(global,factory){typeof exports==='object'&&typeof module!=='undefined'?module.exports=factory():typeof define==='function'&&define.amd?define(factory):(global=global||self,global.Chart=factory())}(this,(function(){'use strict';var commonjsGlobal=typeof globalThis!=='undefined'?globalThis:typeof window!=='undefined'?window:typeof global!=='undefined'?global:typeof self!=='undefined'?self:{};function commonjsRequire(){throw new Error('Dynamic requires are not currently supported by rollup-plugin-commonjs')}
...
'use strict';var countriesHistoryData,countriesCurrentData,currentTotalData,countriesAllData=[],countriesAllCurrentData=[],lastStatisticsTime,todayDate=new Date(),todayDateUse=todayDate.getFullYear()+'-'+(todayDate.getMonth()+1)+'-'+todayDate.getDate(),svgMap;function titleCase(str){var splitStr=str.toLowerCase().split(' ');for(var i=0;i<splitStr.length;i++){splitStr[i]=splitStr[i].charAt(0).toUpperCase()+splitStr[i].substring(1)}
return splitStr.join(' ')}
function mapData(){$.each(countriesHistoryData,function(countryName,countryDaysData){$.each(countriesCurrentData.countries_stat,function(key,countryCurrentData){if((countryCurrentData.country_name===countryName)||(countryName==='United Arab Emirates'&&countryCurrentData.country_name==='Saudi Arabia')||(countryName==='US'&&countryCurrentData.country_name==='USA')||(countryName==='Korea, South'&&countryCurrentData.country_name==='S. Korea')||(countryName==='Taiwan*'&&countryCurrentData.country_name==='Taiwan')||(countryName==='United Kingdom'&&countryCurrentData.country_name==='UK')||(countryName==='Holy See'&&countryCurrentData.country_name==='Vatican City')||(countryName==='Congo (Kinshasa)'&&countryCurrentData.country_name==='Congo')||(countryName==='Congo (Brazzaville)'&&countryCurrentData.country_name==='Congo')||(countryName==='The Bahamas'&&countryCurrentData.country_name==='Bahamas')||(countryName==='Gambia, The'&&countryCurrentData.country_name==='Gambia')||(countryName==='Saint Vincent and the Grenadines'&&countryCurrentData.country_name==='St. Vincent Grenadines')){countryDaysData[countryDaysData.length]={date:todayDateUse,confirmed:parseInt(countryCurrentData.cases.replace(',','')),deaths:parseInt(countryCurrentData.deaths.replace(',','')),recovered:parseInt(countryCurrentData.total_recovered.replace(',','')),new_deaths:parseInt(countryCurrentData.new_deaths.replace(',','')),new_cases:parseInt(countryCurrentData.new_cases.replace(',','')),serious_critical:parseInt(countryCurrentData.serious_critical.replace(',','')),active_cases:parseInt(countryCurrentData.active_cases.replace(',','')),total_cases_per_1m_population:parseInt(countryCurrentData.total_cases_per_1m_population.replace(',',''))}}});countriesAllData[countryName]=countryDaysData});countriesAllCurrentData.World={date:todayDateUse,confirmed:parseInt(currentTotalData.total_cases.replace(',','')),deaths:parseInt(currentTotalData.total_deaths.replace(',','')),recovered:parseInt(currentTotalData.total_recovered.replace(',','')),new_deaths:parseInt(currentTotalData.new_deaths.replace(',','')),new_cases:parseInt(currentTotalData.new_cases.replace(',','')),serious_critical:'N/A',active_cases:'N/A',total_cases_per_1m_population:'N/A'};countriesAllCurrentData.World.active_cases=countriesAllCurrentData.World.confirmed-countriesAllCurrentData.World.deaths-countriesAllCurrentData.World.recovered;countriesAllCurrentData.World.total_cases_per_1m_population=parseInt(countriesAllCurrentData.World.confirmed*1000000/7700000000);updatePanelData('World',countriesAllCurrentData.World)}
function addDataToSelect(){$.each($('.country'),function(){var countryName=$(this).attr('data-value'),countryData,countryStatsHTML='';if(undefined!==countriesAllData[countryName]){countryData=countriesAllData[countryName][countriesAllData[countryName].length-1];countryStatsHTML='<div class="stats"><div class="c">C: <span class="couC">'+countryData.confirmed+'</span></div><div class="d">D: <span class="couD">'+countryData.deaths+'</span></div><div class="r">R: <span class="couR">'+countryData.recovered+'</span></div></div>'}
$(this).append(countryStatsHTML)})}
function initMap(){svgMap=$('.mapWrapper > .svgMapWrapper > svg');$.each(svgMap.find('path'),function(){var countryName=$(this).attr('title'),countryDaysData=countriesAllData[countryName];if(undefined===countryDaysData){if(countryName==='Democratic Republic of Congo'){countryDaysData=countriesAllData.Congo}
...
if(undefined!==countryDaysData){var upto=39;var max=25000;if(countryDaysData[countryDaysData.length-1].confirmed<500){upto=15;max=500}else if(countryDaysData[countryDaysData.length-1].confirmed<5000){upto=30;max=5000}
var colorScheme=((upto*countryDaysData[countryDaysData.length-1].confirmed)/max);if(colorScheme>upto){colorScheme=upto}
colorScheme=40-colorScheme;$(this).css('fill','hsl('+colorScheme+',90%,60%)')}})}
function loadCountryStats(country){var countryName=country,countryDaysData=countriesAllData[countryName];if(undefined===countryDaysData){if(countryName==='Democratic Republic of Congo'){countryDaysData=countriesAllData.Congo}
if(countryName==='Republic of Congo'){countryDaysData=countriesAllData.Congo}
if(countryName==='Czech Republic'){countryDaysData=countriesAllData.Czechia}
if(countryName==='Macedonia'){countryDaysData=countriesAllData['North Macedonia']}
if(countryName==='United States'){countryDaysData=countriesAllData.US}}
if(undefined!==countryDaysData){loadCharts(countryName,countryDaysData)}
return countryDaysData}
var deathsChart,deathsCurrentChart,activeChart,activeCurrentChart,recoveredChart,recoveredCurrentChart;function loadCharts(countryName,data){if(deathsChart){deathsChart.destroy();deathsCurrentChart.destroy();activeChart.destroy();activeCurrentChart.destroy();recoveredChart.destroy();recoveredCurrentChart.destroy()}
var dates=[];var active=[];var activeCurrent=[];var confirmed=[];var confirmedCurrent=[];var recovered=[];var recoveredCurrent=[];var deaths=[];var deathsCurrent=[];var currA=0;var currC=0;var currD=0;var currR=0;$.each(data,function(k,v){dates.push(v.date);confirmed.push(v.confirmed);confirmedCurrent.push(v.confirmed-currC);deaths.push(v.deaths);deathsCurrent.push(v.deaths-currD);recovered.push(v.recovered);recoveredCurrent.push(v.recovered-currR);var activec=v.confirmed-v.deaths-v.recovered;active.push(activec);activeCurrent.push(activec-currA);currC=v.confirmed;currD=v.deaths;currR=v.recovered;currA=activec});updatePanelData(countryName,data[data.length-1]);var globarChartOptions={type:'bar',options:{elements:{point:{radius:0}},legend:{labels:{fontColor:"white",}},scales:{xAxes:[{stacked:!1,ticks:{fontColor:"white"}}],yAxes:[{stacked:!1,ticks:{beginAtZero:!0,fontColor:"white"},gridLines:{color:"rgba(255,255,255,0.05)"}}]},responsive:!0,}};var activectx=document.getElementById('active').getContext('2d');activeChart=new Chart(activectx,{...globarChartOptions,data:{labels:dates,datasets:[{label:'Total Cases',backgroundColor:'rgba(250,200,0,0.0)',hoverBackgroundColor:'rgba(250,200,0,0.6)',data:confirmed,},{label:'',data:confirmed,backgroundColor:'rgba(250,200,0,0.6)',borderColor:'#fbc500',type:'line',fill:!0,},{label:'Active Cases',backgroundColor:'rgba(255, 106,0,0.0)',hoverBackgroundColor:'rgba(255, 106,0,0.6)',data:active,},{label:'',data:active,backgroundColor:'rgba(255, 106,0,0.6)',borderColor:'#ff6a00',type:'line',fill:!0},]},});var activeCctx=document.getElementById('activeC').getContext('2d');activeCurrentChart=new Chart(activeCctx,{...globarChartOptions,data:{labels:dates,datasets:[{label:'Confirmed Every Day',backgroundColor:'rgba(255,167,0,0)',hoverBackgroundColor:'rgba(255,167,0,0.6)',borderColor:'#FF9400',data:confirmedCurrent,},{label:'',data:confirmedCurrent,backgroundColor:'rgba(255,167,0,0.6)',borderColor:'#FF9400',type:'line',fill:!0,}]},});var recoveredctx=document.getElementById('recovered').getContext('2d');recoveredChart=new Chart(recoveredctx,{...globarChartOptions,data:{labels:dates,datasets:[{label:'Total Recovered',backgroundColor:"rgba(58,255,0,0)",hoverBackgroundColor:"rgba(58,255,0,0.6)",borderColor:'#3AFF00',data:recovered,},{label:'',data:recovered,backgroundColor:'rgba(58,255,0,0.6)',borderColor:'#3AFF00',type:'line',fill:!0,}]},});var recoveredCctx=document.getElementById('recoveredC').getContext('2d');recoveredCurrentChart=new Chart(recoveredCctx,{...globarChartOptions,data:{labels:dates,datasets:[{label:'Recovered Every Day',backgroundColor:"rgba(58,255,0,0)",hoverBackgroundColor:"rgba(58,255,0,0.6)",borderColor:'#3AFF00',data:recoveredCurrent,},{label:'',data:recoveredCurrent,backgroundColor:'rgba(58,255,0,0.6)',borderColor:'#3AFF00',type:'line',fill:!0,}]},});var deathsctx=document.getElementById('deaths').getContext('2d');deathsChart=new Chart(deathsctx,{...globarChartOptions,data:{labels:dates,datasets:[{label:'Total Deaths',backgroundColor:'rgba(255,0,0,0)',hoverBackgroundColor:"rgba(255,0,0,0.6)",borderColor:'#ff0000',data:deaths,},{label:'',data:deaths,backgroundColor:'rgba(255,0,0,0.6)',borderColor:'#ff0000',type:'line',fill:!0,}]},});var deathsCctx=document.getElementById('deathsC').getContext('2d');deathsCurrentChart=new Chart(deathsCctx,{...globarChartOptions,data:{labels:dates,datasets:[{label:'Deaths Every Day',backgroundColor:'rgba(255,0,0,0)',hoverBackgroundColor:"rgba(255,0,0,0.6)",borderColor:'#ff0000',data:deathsCurrent,},{label:'',data:deathsCurrent,backgroundColor:'rgba(255,0,0,0.6)',borderColor:'#ff0000',type:'line',fill:!0,}]},})}
function updatePanelData(countryName,lastStats){$('.timeFetched').text(lastStatisticsTime);$('.countryN').text(countryName);$('.totalC').text(lastStats.confirmed);$('.totalA').text(lastStats.active_cases);$('.totalR').text(lastStats.recovered);$('.totalD').text(lastStats.deaths);$('.newC').text(lastStats.new_cases);$('.newD').text(lastStats.new_deaths);$('.perM').text(lastStats.total_cases_per_1m_population)}
function doTheInit(){if(undefined===countriesHistoryData){setTimeout(doTheInit,1);return}
if(undefined===countriesCurrentData){setTimeout(doTheInit,1);return}
if(undefined===currentTotalData){setTimeout(doTheInit,1);return}
lastStatisticsTime=countriesCurrentData.statistic_taken_at;mapData();addDataToSelect();initMap();$(document).ready(function(){var body=$('body');body.on('click','.openMap',function(){body.find('.mapWrapper').toggleClass('active');body.find('.openMap').toggleClass('active');body.toggleClass('panel-active')});svgMap.on('mouseenter','path',function(){var countryName=$(this).attr('title'),countryDaysData=countriesAllData[countryName];if(undefined===countryDaysData){if(countryName==='Democratic Republic of Congo'){countryDaysData=countriesAllData.Congo}
if(countryName==='Republic of Congo'){countryDaysData=countriesAllData.Congo}
if(countryName==='Czech Republic'){countryDaysData=countriesAllData.Czechia}
if(countryName==='Macedonia'){countryDaysData=countriesAllData['North Macedonia']}
if(countryName==='United States'){countryDaysData=countriesAllData.US}}
if(undefined!==countryDaysData){updatePanelData(countryName,countryDaysData[countryDaysData.length-1])}else{updatePanelData('World',countriesAllCurrentData.World)}});svgMap.on('mouseleave','path',function(){updatePanelData('World',countriesAllCurrentData.World)});svgMap.on('click','path',function(){var countryName=$(this).attr('title');var countryDaysData=loadCountryStats(countryName);if(undefined!==countryDaysData){body.toggleClass('panel-active');body.find('.mapWrapper').toggleClass('active');body.find('.openMap').toggleClass('active');setTimeout(function(){updatePanelData(countryName,countryDaysData[countryDaysData.length-1])},1)}});$('#worldMap').svgPanZoom();body.on('click','.country',function(){$('.country').removeClass('selected');$(this).addClass('selected');var countryName=$(this).attr('data-value');updatePanelData(countryName,countriesAllData[countryName][countriesAllData[countryName].length-1])});body.on('click','.show-stats-btn',function(){if(body.hasClass('stats-active')){body.toggleClass('stats-active');$(this).text('Show Statistics')}else{var countryName=$('.country.selected').attr('data-value');var countryDaysData=loadCountryStats(countryName);if(undefined!==countryDaysData){body.toggleClass('stats-active');$(this).text('Show Countries')}}});body.on('keyup touchdown change','input.autocomplete',function(){$('.country').removeClass('matched not-matched');if($(this).val().length===0){}else{var matched=$('.country[data-value*='+titleCase($(this).val())+']');matched.addClass('matched');var notMatched=$('.country').not('.matched');notMatched.addClass('not-matched')}})})}
fetch("https://pomber.github.io/covid19/timeseries.json").then(response=>response.json()).then(data=>{countriesHistoryData=data});fetch("https://coronavirus-monitor.p.rapidapi.com/coronavirus/cases_by_country.php",{"method":"GET","headers":{"x-rapidapi-host":"coronavirus-monitor.p.rapidapi.com","x-rapidapi-key":"e6ffbc337emshf94beef1a5be392p10ad41jsn44eda845f48e"}}).then(response=>response.json()).then(data=>{countriesCurrentData=data});fetch("https://coronavirus-monitor.p.rapidapi.com/coronavirus/worldstat.php",{"method":"GET","headers":{"x-rapidapi-host":"coronavirus-monitor.p.rapidapi.com","x-rapidapi-key":"e6ffbc337emshf94beef1a5be392p10ad41jsn44eda845f48e"}}).then(response=>response.json()).then(data=>{currentTotalData=data});doTheInit()
